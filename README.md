...
# 1.6.25
### 1.新增提供商生成二维码直接保存到本地功能，优化二维码生成逻辑
### 2.优化设置-提供商-任意提供商-APi Key的显示，现在支持点击Api Key输入框右边的小眼睛图标修改是否模糊化Api Key的显示功能
### 3.在开启MCP服务的时候在点击对话框中底部的MCP小图标后弹出的页面的MCP服务器文字的右边添加一个入口按钮，点击后跳转到设置的MCP设置中，这样可以让用户更方便

# 1.6.26
### 1.新增底部翻译按钮
### 2.新增聊天记录导入导出功能
### 3.修改更新信息显示逻辑，现在更新信息不会自动跑到用户信息栏上面，而是跟着聊天记录一起上下浮动
### 4.在显示设置中，新增自定义输入栏按钮，可以调整前后顺序和是否显示
### 5.新增提供商二维码分享携带提供商密钥及模型列表导出功能
### 6.修复在从相册选择提供商的时候如果设备上已经有了这个提供商再去导入它就会直接闪退的问题
### 7.修改更新逻辑，使用wanxiaoT的服务器处理更新，再也不会串味了
